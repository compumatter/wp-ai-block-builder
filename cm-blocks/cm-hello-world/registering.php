...
function cm_hello_world_register_all_assets() {
    $asset_handles = CM_Hello_World_Config::get_asset_handles();
    $file_paths = CM_Hello_World_Config::get_file_paths();
    ...
    $js_config = CM_Hello_World_Config::get_javascript_constants();
    ...
    wp_add_inline_style(
        $asset_handles['centralized_css'],
        ':root { ' . CM_Hello_World_Config::build_global_css_properties() . ' }'
    );
}

// Register block with proper asset integration
function cm_hello_world_register_block() {
    cm_hello_world_register_all_assets();
    
    $asset_handles = CM_Hello_World_Config::get_asset_handles();
    
    register_block_type(get_template_directory() . '/cm-blocks/cm-hello-world', array(
        'editor_script' => $asset_handles['editor_js']
    ));
}

// WordPress hooks
add_action('init', 'cm_hello_world_register_block');
